...
abstract class AggregationTransformer(override val uid: String) extends PipelineStage
{
	...
	/**
		...
		* @return
		*/
	def setInputCols(values: Seq[String]) = set( inputCols, values )
...
object AggregationTransformer
{
	/**
		...
		* @return
		*/
	def simpleAggTx(inputCols : Seq[String],
	                outputCol : String,
	                outputType: DataType,
	...
	/**
		...
		* @return
		*/
	def simpleAggTx(inputCol  : String,
	                outputCol : String,
	                outputType: DataType,
	                aggFn     : Column => Column): AggregationTransformer =
		simpleAggTx( Seq( inputCol ), outputCol, outputType, (in: Seq[Column]) => aggFn( in( 0 ) ) )

	def UDAFAsTx(inputCols: Seq[String],
	             outputCol: String,
	...
	def UDAFAsTx(inputCol : String,
	             outputCol: String,
	             udaf     : UserDefinedAggregateFunction): AggregationTransformer =
		UDAFAsTx( Seq( inputCol ), outputCol, udaf )
}
